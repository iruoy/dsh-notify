...
export function validateBaseUrl(value) {
    ...
    if (typeof value !== 'string' || value.length > 2048)
        throw new ValidationError('Invalid DSH base URL.');
    let url;
    try {
        url = new URL(value);
    }
...
export function validateWebhook(value) {
    if (typeof value !== 'string' || value.length > 1024)
        throw new ValidationError('Invalid Slack webhook.');
    let url;
    try {
        url = new URL(value);
    }
...
export function validateSettings(value) {
    ...
    const events = (v) => {
        ...
        return Object.fromEntries(KINDS.map(k => [k, bool(k === 'question' && input[k] === undefined ? true : input[k])]));
    };
