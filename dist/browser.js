...
export class BrowserStream {
    ...
    connect(res, cursor) {
        ...
        if (cursor !== undefined) {
            ...
            if (cursor < oldest - 1 || cursor > this.store.state.sequence)
                this.write(res, 'event: gap\ndata: {"message":"Replay window exceeded; check recent deliveries."}\n\n');
            for (const entry of this.store.state.history)
                if (entry.seq > cursor && this.eligible(entry))
