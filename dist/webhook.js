...
export function slackPayload(notice, baseUrl) {
    ...
    if (notice.workspace)
        details.push(`Workspace: ${notice.workspace}`);
    ...
    if (notice.input)
        blocks.push({ type: 'section', text: { type: 'plain_text', text: `Input:\n${notice.input.slice(0, 1500)}` } });
    if (notice.runs?.length) {
        ...
        const runs = notice.runs.map(run => {
            ...
            if (run.reportedCalls) {
                ...
                if (run.cacheReadTokens || run.cacheWriteTokens)
                    lines.push(`Cache: ${count(run.cacheReadTokens)} read · ${count(run.cacheWriteTokens)} write`);
            }
            else
                lines.push('Tokens: Not reported');
            return lines.join('\n');
        });
        // Keep each section within Slack's limits, including unusually long route names.
        for (const run of runs.slice(0, 40))
            blocks.push({ type: 'section', text: { type: 'plain_text', text: run.slice(0, 2900) } });
        if (runs.length > 40)
            blocks.push({ type: 'section', text: { type: 'plain_text', text: `${runs.length - 40} additional model/effort combinations omitted.` } });
        if (!notice.usageComplete)
            blocks.push({ type: 'context', elements: [{ type: 'plain_text', text: 'Token usage is partial; some calls did not report usage.' }] });
    }
    if (notice.summary)
        blocks.push({ type: 'section', text: { type: 'plain_text', text: notice.summary.slice(0, 1500) } });
    if (notice.cost?.pricedCalls && notice.cost.fetchedAt !== undefined)
        blocks.push({ type: 'context', elements: [{ type: 'plain_text', text: `Models.dev public list prices · refreshed ${new Date(notice.cost.fetchedAt).toISOString().slice(0, 10)}${notice.cost.stale ? ' · stale cache' : ''}` }] });
